...
namespace App\Api\Http\Response\Format;
...
class Json extends Format
{
    ...
    public function formatEloquentModel($model)
    {
        if($this->response->isOk())
            return $this->encode(["status"=>"ok","payload" => $model->toArray()]);
    ...
    public function formatEloquentCollection($collection)
    {
        if ($collection->isEmpty()) {
            return $this->encode(["status"=>"ok","payload"=>[]]);
        }
        if($this->response->isOk())
            return $this->encode(["status"=>"ok","payload" => $collection->toArray()]);
    ...
    public function formatArray($content)
    {
        ...
        if($this->response->isOk())
            return $this->encode(["status"=>"ok","payload" => $content]);
